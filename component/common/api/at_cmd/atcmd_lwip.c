				...
				if(recv_size){
					...
					if(curnode->protocol == NODE_MODE_UDP && curnode->role == NODE_ROLE_SERVER){
						...
						at_printf("\r\n[ATPR] OK,%d,%d,%s,%d:", recv_size, curnode->con_id, udp_clientaddr, udp_clientport);
                                            ...
                                            if(curnode->protocol == NODE_MODE_TCP && curnode->role == NODE_ROLE_SEED){
                                                struct sockaddr_in cli_addr;
                                                error_no = atcmd_lwip_send_data(curnode, rx_buffer, recv_size, cli_addr);
                                                if(error_no){
                                                    at_printf("\r\n[ECHO] ERROR:%d,%d", error_no, curnode->con_id);
                                                    at_printf(STR_END_OF_ATCMD_RET);
                                                }
                                            }else{
                                                AT_DBG_MSG(AT_FLAG_LWIP, AT_DBG_ALWAYS,
